<?php
	...
	if(isset($_POST)){
		...
		if(!$result = $db->query("UPDATE tblBuilding SET favorite = $favorite WHERE buildingID =$buildingID")){
			die('There was an error running the query [' . $db->error . ']');
		}

		// else if(!$result = $db->query("INSERT INTO tblBuilding (buildingID, ownerID, address, numRooms, bankNum, routingNum, imageUrl) VALUES($buildingID, $ownerID, '$address', $numRooms, $bankNum, $routingNum, '$imageUrl')")){
		// 	die('There was an error running the query [' . $db->error . ']');
		// }
